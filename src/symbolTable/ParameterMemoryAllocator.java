...
public class ParameterMemoryAllocator implements MemoryAllocator {
	...
	List<MemoryLocation> allocatedMemories;
	...
	public ParameterMemoryAllocator(MemoryAccessMethod accessor, String baseAddress, int startingOffset) {
		...
		this.allocatedMemories = new ArrayList<MemoryLocation>();
	}
	...
	public MemoryLocation allocate(int sizeInBytes) {
		...
		allocatedMemories.add(memoryLocation);
	...
	public void restoreState() {
		...
		if( bookmarks.size() == 0) {
			// add maximum allocated size to all memory locations it has allocated
			for(MemoryLocation memoryLocation : allocatedMemories) {
				memoryLocation.addOffset(getMaxAllocatedSize()); 
			}
		}
	}
}
